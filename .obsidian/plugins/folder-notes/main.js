...
var SettingsTab = class extends import_obsidian6.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian6.Setting(containerEl).setName("Hide folder note").setDesc("Hide the folder note in the file explorer").addToggle((toggle) => toggle.setValue(this.plugin.settings.hideFolderNote).onChange(async (value) => {
    ...
    new import_obsidian6.Setting(containerEl).setName("Sync folder name").setDesc("Automatically rename the folder note when the folder name is changed").addToggle((toggle) => toggle.setValue(this.plugin.settings.syncFolderName).onChange(async (value) => {
      this.plugin.settings.syncFolderName = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian6.Setting(containerEl).setName("Auto create folder note").setDesc("Automatically create a folder note when a new folder is created").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoCreate).onChange(async (value) => {
      this.plugin.settings.autoCreate = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    if (import_obsidian6.Platform.isDesktopApp) {
      new import_obsidian6.Setting(containerEl).setName("Key for creating folder note").setDesc("The key combination to create a folder note").addDropdown((dropdown) => {
    ...
    new import_obsidian6.Setting(containerEl).setName("Add underline to folders with folder notes").setDesc("Add an underline to folders that have a folder note in the file explorer").addToggle((toggle) => toggle.setValue(this.plugin.settings.underlineFolder).onChange(async (value) => {
    ...
    new import_obsidian6.Setting(containerEl).setName("Underline folders with folder notes in the path").setDesc("Add an underline to folders that have a folder note in the path").addToggle((toggle) => toggle.setValue(this.plugin.settings.underlineFolderInPath).onChange(async (value) => {
    ...
    new import_obsidian6.Setting(containerEl).setName("Open folder note through path").setDesc("Open the folder note when clicking on a folder name in the path").addToggle((toggle) => toggle.setValue(this.plugin.settings.openFolderNoteOnClickInPath).onChange(async (value) => {
    ...
    new import_obsidian6.Setting(containerEl).setHeading().setName("Manage excluded folders");
    new import_obsidian6.Setting(containerEl).setName("Add excluded folder").addButton((cb) => {
      cb.setIcon("plus");
...
var FolderNameModal = class extends import_obsidian7.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian7.Setting(contentEl).setName("Enter the name of the folder").addText((text) => text.setValue(this.folder.name.replace(".md", "")).onChange(async (value) => {
      if (value.trim() !== "") {
    ...
    this.plugin.registerEvent(this.app.workspace.on("file-menu", (menu, file) => {
      ...
      if (this.plugin.settings.excludeFolders.find((folder) => folder.path === file.path)) {
        menu.addItem((item) => {
          item.setTitle("Remove folder from excluded folders").setIcon("trash").onClick(() => {
            ...
            this.plugin.saveSettings();
            new import_obsidian8.Notice("Successfully removed folder from excluded folders");
          });
        });
        return;
      }
      menu.addItem((item) => {
        item.setTitle("Exclude folder from folder notes").setIcon("x-circle").onClick(() => {
          ...
          this.plugin.settings.excludeFolders.push(excludedFolder);
          this.plugin.saveSettings();
          new import_obsidian8.Notice("Successfully excluded folder from folder notes");
        });
      });
      const folderNote = this.plugin.app.vault.getAbstractFileByPath(file.path + "/" + file.name + ".md");
      if (folderNote && folderNote instanceof import_obsidian8.TFile) {
        menu.addItem((item) => {
...
// src/main.ts
var FolderNotesPlugin = class extends import_obsidian10.Plugin {
  async onload() {
      ...
      mutations.forEach((rec) => {
        if (rec.type === "childList") {
          rec.target.querySelectorAll("div.nav-folder-title-content").forEach((element) => {
            if (element.onclick)
              return;
            element.onclick = (event) => this.handleFolderClick(event);
          });
            ...
            if (breadcrumbs.length > 0) {
              breadcrumbs.forEach((breadcrumb) => {
                if (breadcrumb.onclick)
                  return;
                breadcrumb.onclick = (event) => this.handleViewHeaderClick(event);
              });
            }
          });
        }
  ...
  async handleFolderClick(event) {
    ...
    const folder = (_a = event.target.parentElement) == null ? void 0 : _a.getAttribute("data-path");
    if (!folder) {
      return;
    }
    ...
    if (excludedFolder == null ? void 0 : excludedFolder.disableFolderNote) {
      ...
      event.target.click();
      return;
    } else if ((excludedFolder == null ? void 0 : excludedFolder.enableCollapsing) || this.settings.enableCollapsing) {
      event.target.onclick = null;
      event.target.click();
    }
    ...
    } else if (event.altKey || import_obsidian10.Keymap.isModEvent(event) === "tab") {
      if (this.settings.altKey && event.altKey || this.settings.ctrlKey && import_obsidian10.Keymap.isModEvent(event) === "tab") {
  ...
  async handleViewHeaderClick(event) {
    if (!(event.target instanceof HTMLElement))
      return;
  ...
  handleFolderRename(file, oldPath) {
    ...
    if (!folder)
      return;
    const excludedFolders = this.settings.excludeFolders.filter((excludedFolder2) => excludedFolder2.path.includes(oldPath));
    ...
    if (excludedFolder == null ? void 0 : excludedFolder.disableSync)
      return;
    ...
    if (!note)
      return;
    note.path = folder.path + "/" + oldFileName + ".md";
  ...
  handleFileRename(file, oldPath) {
    ...
    if (oldFolder) {
      ...
      if ((newExcludedFolder == null ? void 0 : newExcludedFolder.disableSync) || (newExcludedFolder == null ? void 0 : newExcludedFolder.disableFolderNote)) {
        return;
      }
    }
  ...
  async renameFolderOnFileRename(file, oldPath, oldFolder) {
    ...
    if (this.app.vault.getAbstractFileByPath(newFolderPath)) {
      ...
      return new import_obsidian10.Notice("A folder with the same name already exists");
    }
  ...
  async createFolderNote(path, openFile, useModal) {
  ...
  async openFolderNote(path, evt) {
    const leaf = this.app.workspace.getLeaf(import_obsidian10.Keymap.isModEvent(evt));
    const file = this.app.vault.getAbstractFileByPath(path);
    if (file instanceof import_obsidian10.TFile) {
      await leaf.openFile(file);
    }
  }
  async deleteFolderNote(file) {
    if (this.settings.showDeleteConfirmation) {
      return new DeleteConfirmationModal(this.app, this, file).open();
    }
    await this.app.vault.delete(file);
  }
  getNameFromPathString(path) {
